...
<div class="mb-8 p-6 bg-white border border-stone-200/80 rounded-2xl shadow-sm">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <div class="rounded-xl border border-amber-200/60 bg-white p-6 shadow-sm">
                        <div class="flex flex-wrap items-start justify-between gap-4 border-b border-stone-100 pb-4">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <a id="btnEditCurrentVideo" href="{{ optional($videos->first())->id ? route('admin.gosali.edit', optional($videos->first())->id) : '#' }}" class="rounded-lg border border-amber-300 bg-amber-50 px-3.5 py-2 text-xs font-bold text-amber-900 transition hover:bg-amber-100 flex items-center gap-1.5 shadow-sm">
                                    ✏️ Edit Babak Ini
                                </a>
                        ...
                        <div id="playlistList" class="max-h-[520px] divide-y divide-stone-100 overflow-y-auto">
                            @forelse($videos as $video)
                                ...
                                <div
                                    ...
                                    onclick="playVideo(this, '{{ $videoSource }}', {{ json_encode($video->title ?? '') }}, {{ json_encode($video->description ?? '') }}, {{ json_encode($video->duration ?? '') }}, '{{ $posterSource }}', '{{ $youtubeEmbedUrl }}', '{{ $video->id }}')"
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-start justify-between gap-2">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('admin.gosali.edit', $video->id) }}" title="Edit Babak Ini" class="text-[10px] font-semibold text-amber-700 hover:text-amber-900">✏️</a>
    ...
    <div id="modalTambahVideo" class="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 p-4 opacity-0 invisible transition-all duration-300 backdrop-blur-sm">
        <div class="w-full max-w-lg max-h-[85vh] flex flex-col scale-95 rounded-xl border border-amber-200 bg-white shadow-2xl transition-all duration-300 overflow-hidden">
            ...
            <form action="{{ route('admin.gosali.store') }}" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto space-y-4 p-6 text-xs text-stone-700">
                @csrf
    ...
    <script>
        ...
        function playVideo(element, videoSrc, title, description, duration, posterSrc, youtubeEmbedUrl, videoId) {
            ...
            if (btnEdit && videoId) {
                btnEdit.href = '/admin/gosali-admin/' + videoId + '/edit';
            }
